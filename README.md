...
**هسته Rust منطق تجاری است — نه Kotlin.** از این به بعد، تمام قوانین، محاسبات، اعتبارسنجی‌ها و تبدیل‌های داده باید ابتدا در **هسته Rust** (`rust/hesabyar-core`) پیاده‌سازی شوند. لایه Kotlin (ViewModel، Repository، UseCase) فقط هماهنگ‌کننده و لایه دسترسی به Android است.

پیاده‌سازی‌های جایگاهی (fallback) در Kotlin فقط برای لیست استثناهای از پیش تأیید‌شده مجاز هستند و به عنوان اینترافیس اضطراری عمل می‌کنند — نه جایی برای منطق جدید:

- تبدیل تقویم جلالی
- قالب‌بندی پول
- پارسر NLP آفلاین
- تجزیه و اعتبارسنجی JSON بکاپ
- اعتبارسنجی خروجی هوش مصنوعی

هر درخواست Pull شامل منطق تجاری جدید در Kotlin (فارغ از لیست استثناها) باید در بازبینی علامت بزند و به سمت Rust هدایت شود. برای جزئیات کامل به `AGENTS.md`، `docs/architecture/ARCHITECTURE.md`، و `docs/architecture/ADR-001-rust-sole-implementation.md` مراجعه کنید.
