...
**سیاست منطق تجاری:** هسته Rust (`rust/hesabyar-core`) تنها مکان پیاده‌سازی منطق تجاری، محاسبات، قوانین و اعتبارسنجی‌های جدید است. هر درخواست Pull که شامل منطق تجاری، محاسبه، یا اعتبارسنجی جدید مستقیماً در Kotlin باشد باید در بازبینی علامت زده شود و به سمت Rust هدایت شود. لیست استثناها و جزئیات کامل سیاست در [سیاست منطق تجاری (Rust-first)](docs/architecture/ADR-001-rust-sole-implementation.md) مرجع شود. در غیر این صورت، این پیاده‌سازی‌ها باید از بین بروند. برنامه حذف تدریجی fallbackهای غیر-استثنا در `plans/2026-08-19-rust-fallback-consolidation-plan.md` مرجع شود.
