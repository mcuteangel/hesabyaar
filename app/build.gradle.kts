...
fun buildHostLibrary() {
  ...
  if (cargoBuild.start().waitFor() != 0) throw GradleException("cargo build --release failed")
}
...
fun resolveHostArtifact(): File {
  ...
  return when {
    ...
    if (!hostLib.exists()) throw GradleException("Host library not found at: ${hostLib.absolutePath}")
    logger.lifecycle("Step 2/4: Host library at ${hostLib.name}")
  }
}

fun generateBindings(hostLib: File, outDir: File) {
  logger.lifecycle("Step 3/4: Generating UniFFI Kotlin bindings...")
  if (outDir.exists()) outDir.deleteRecursively()
  outDir.mkdirs()
  val exitCode = runUniffiGen(hostLib, outDir)
  if (exitCode != 0) throw GradleException("UniFFI binding generation failed")
}

fun patchAndInstallOutput(tempDir: File, dest: File) {
  logger.lifecycle("Step 4/4: Patching package and installing bindings...")
  val generatedKt = tempDir.walkTopDown().firstOrNull { it.isFile && it.extension == "kt" }
...
val versionString = if (versionFile.exists()) {
  ...
  throw GradleException("VERSION file not found at: ${versionFile.absolutePath}. Please create a VERSION file with format MAJOR.MINOR.PATCH (e.g., 0.1.0)")
}
val versionParts = versionString.split(".")
if (versionParts.size != 3) {
  throw GradleException("VERSION file must contain exactly 3 dot-separated segments (MAJOR.MINOR.PATCH), got: '$versionString'")
}
val calculatedVersionCode = try {
  versionParts[0].toInt() * 10000 + versionParts[1].toInt() * 100 + versionParts[2].toInt()
} catch (e: NumberFormatException) {
  throw GradleException("VERSION file must contain only numeric segments (MAJOR.MINOR.PATCH), got: '$versionString'", e)
}
...
tasks.withType<org.gradle.api.tasks.testing.Test>().configureEach {
  jvmArgs("-Djna.library.path=${rustReleaseDir.absolutePath}", "-Djava.library.path=${rustReleaseDir.absolutePath}")
}
...
data class RustTarget(val abi: String, val triple: String, val jniLib: String)
...
rustTargets.forEach { target ->
  ...
  tasks.register(taskName) {
    ...
    doLast {
      ...
      if (ndkHome.isNullOrBlank()) throw GradleException("ANDROID_NDK_HOME is not set. Install the Android NDK and set ANDROID_NDK_HOME to its root directory. Example: export ANDROID_NDK_HOME=~/Android/Sdk/ndk/27.0.12077973")
      if (outDir.exists()) outDir.deleteRecursively()
      outDir.mkdirs()
      ...
      val foundLib = outDir.walkTopDown().firstOrNull { it.name == target.jniLib } ?: throw GradleException("Expected native library ${target.jniLib} not found for ${target.abi} at ${outDir.absolutePath}")
      if (foundLib != outputLib) {
        foundLib.copyTo(outputLib, overwrite = true)
        val nestedDir = File(outDir, target.abi)
        if (nestedDir.exists() && nestedDir != outputLib.parentFile) nestedDir.deleteRecursively()
      }
    }
  }
}
...
tasks.register("generateKeystore") {
  ...
  doFirst {
    val storePassword = resolveCredential(storePwdKey)
    val keyPassword = resolveCredential(keyPasswordKey)
    val keyAlias = providers.gradleProperty(keyAliasKey).orNull ?: providers.environmentVariable(keyAliasKey).orNull ?: "mojrico"
    if (storePassword.isBlank() || keyPassword.isBlank()) throw GradleException("$storePwdKey and $keyPasswordKey must be set. Add them to your local .env file or set as environment variables. See .env.example for reference.")
    val keystoreFile = File(rootDir, "my-upload-key.jks")
    if (!keystoreFile.exists()) {
      println("Generating release keystore...")
      val pb = ProcessBuilder("keytool", "-genkey", "-noprompt", "-alias", keyAlias, "-dname", "CN=Hesabyar, OU=None, O=None, L=None, S=None, C=IR", "-keystore", keystoreFile.absolutePath, "-storepass", storePassword, "-keypass", keyPassword, "-keyalg", "RSA", "-keysize", "2048", "-validity", "10000")
      val proc = pb.start()
      proc.waitFor()
