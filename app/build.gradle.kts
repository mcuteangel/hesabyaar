url: https://raw.githubusercontent.com/mcuteangel/hesabyaar/fix/version-reading/app/build.gradle.kts
...
fun patchAndInstallOutput(
  ...
  logger.lifec
ycle("Step 4/4: Patching package and installing bindings...")
  val generatedKt =
    tempDir
      .walkTopDown()
...
android {
  ...
  signingConfigs {
    create("release") {
      ...
      keyPassword = resolveCredential(k
eyPasswordKey)
    }
  }
...
tasks.register("checkSigningConfig") {
  ...
  doFirst {
    ...
    if (issues.isNotEmpty()) {
      logger.warn("warnin
g Signing configuration issues:")
...
dependencies {
  ...
  implementation(libs.kotlinx.corou
tines.core)
...
val rustTargets = listOf(
  ...
  RustTarget("x86_64", "x86_64-linu
x-android", "libhesabyar_core.so"),
...
tasks.register("generateRustBindings") {
  ...
  doLast {
    ...
    logger.lifecycle("Kotlin bindings generated at: ${generatedDir.absoluteP
ath}")
  }
}
