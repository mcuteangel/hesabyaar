...
- [ ] 5. If exposed to Kotlin, add `#[uniffi::export]` and update `app/buildSrc/template/HesabyarCore.template.kt` if signature requires defaults.
- [ ] 6. Regenerate UniFFI bindings: `./gradlew --no-daemon :app:generateAndFixBindings --rerun-tasks`
- [ ] 7. Write Kotlin caller in the appropriate UseCase via `RustBridge`.
- [ ] 8. Write Kotlin unit test for the UseCase caller.
- [ ] 9. Run isolated bridge tests: `./gradlew --no-daemon testDebugUnitTestRust --rerun-tasks`
